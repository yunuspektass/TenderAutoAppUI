<template>
  <v-container v-if="!loading" fluid>
    <v-card>
      ...
      <v-card-subtitle>
        Sorumlu olduğunuz ihaleleri görüntüleyebilir ve düzenleyebilirsiniz.
      </v-card-subtitle>

      <v-tabs v-model="activeTab" background-color="transparent" grow>
        <v-tab v-for="tender in userTenders" :key="tender.id">
          {{ tender.title }}
        </v-tab>
      </v-tabs>

      <v-tabs-items v-model="activeTab">
        <v-tab-item v-for="tender in userTenders" :key="tender.id">
          <v-card flat>
            <v-card-text>
              <v-row>
                <v-col cols="12" sm="6">
...
export default {
  ...
  computed: {
    ...
    userTenders () {
      return this.tenders.filter(tender => this.userTenderIds.includes(tender.id))
    }
  },
  watch: {
  ...
  methods: {
    ...
    openEditTenderDialog (tender) {
      ...
      this.existingUserIds = this.getUserTendersByTenderId(tender.id).map(userTender => userTender.userId)
      this.dialog = true
    },
    closeDialog () {
      this.dialog = false
