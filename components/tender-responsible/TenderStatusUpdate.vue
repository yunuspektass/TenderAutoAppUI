    ...
    <v-card class="mb-6">
      ...
      <v-data-table :headers="finishedTendersHeaders" :items="getFinishedTenders" class="elevation-1">
        <template #[`item.awardedAmount`]="{ item }">
          {{ item.awardedAmount ? formatCurrency(item.awardedAmount) : 'Veri yok' }}
        </template>
...
export default {
  ...
  computed: {
    ...mapGetters({
      ...
      getCompanyTenderByTenderAndCompanyId: 'companyTender/getCompanyTenderByTenderAndCompanyId'
    }),
    getActiveTenders () {
    ...
    async selectWinner (tenderId, offerId) {
      ...
      try {
        ...
        offers.forEach(async (offer) => {
          if (offer.id !== offerId) {
            await this.$store.dispatch('companyTender/updateCompanyTender', {
